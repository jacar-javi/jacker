...
# Note: Using command categories instead of individual renamed commands
# Command categories are evaluated AFTER rename-command directives in redis.conf
cat > /usr/local/etc/redis/users.acl << EOF
user default on >${DEFAULT_PASSWORD} +@all ~* &*
user oauth_user on >${OAUTH_PASSWORD} -@all +@read +@write +@keyspace +@connection +eval +evalsha +script ~* resetchannels
user ratelimit_user on >${RATELIMIT_PASSWORD} -@all +@read +@write +@keyspace +@connection +select +eval +evalsha +script ~* resetchannels
