...
# Skip individual compose file validation as they require the full context
# (networks, volumes, etc. defined in docker-compose.yml)
# The main docker-compose.yml validation above already validates all included files
test_skip "Individual compose files require full context"
...
# .env files are generated by setup.sh, templates not required in repository
if [ -f .env.sample ] || [ -f .env.template ] || [ -f .env.defaults ]; then
    test_pass
else
    test_fail ".env template" "No .env template or .env.defaults found"
fi
...
# These numbered scripts were removed in recent refactoring (see git status)
# setup.sh now handles installation directly
# Skip this test as the architecture has changed
test_skip "Numbered asset scripts removed in refactoring"
...
test_start "No :latest tags in compose files (except socket-proxy)"
# socket-proxy is explicitly allowed to use :latest for security updates
